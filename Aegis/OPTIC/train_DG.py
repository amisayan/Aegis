    ...
    def run(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch, data in enumerate(self.train_loader):
                ...
                pred_freq,pred_ori = self.model(x)

                y = torch.cat((y, y), dim=0)
                pred = torch.cat((pred_freq,pred_ori), dim=0)
                ...
                uncertainty_map_1 = compute_entropy_uncertainty(prob_map[:, 0:1, :, :])
                uncertainty_map_2 = compute_entropy_uncertainty(prob_map[:, 1:2, :, :])
                uncertainty_map_1 = uncertainty_map_1.unsqueeze(1)
                uncertainty_map_2 = uncertainty_map_2.unsqueeze(1)
                uncertainty_map = torch.cat([uncertainty_map_1, uncertainty_map_2], dim=1)
            ...
            if (epoch + 1) % self.valid_frequency == 0 and self.valid_loader is not None:
                ...
                if (result_dict['Disc_Dice'] + result_dict['Cup_Dice']) / 2 > best_dice:
                    best_dice = (result_dict['Disc_Dice'] + result_dict['Cup_Dice']) / 2
                    best_epoch = (epoch + 1)
                    if torch.cuda.device_count() > 1:
                        torch.save(self.model.module.state_dict(),
