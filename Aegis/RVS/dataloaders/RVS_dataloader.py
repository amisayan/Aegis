...
def get_seg_dg_dataloader(dataroot, Source_Dataset, Target_Dataset, batch_size, workers):
    transform_train, transform_test = get_dg_segtransform()

    trainset = RetinalVesselSegmentation(dataroot, phase='train', splitid=Source_Dataset, transform=transform_train)
    testset = RetinalVesselSegmentation(dataroot, phase='test', splitid=Target_Dataset, transform=transform_test)

    train_loader = torch.utils.data.DataLoader(
        trainset, batch_size=batch_size, shuffle=True, num_workers=workers,
        pin_memory=True, drop_last=True, collate_fn=train_dg_collate_fn
    )
    test_loader = torch.utils.data.DataLoader(
        testset, batch_size=1, shuffle=False, num_workers=workers, pin_memory=True,
        drop_last=False, collate_fn=test_dg_collate_fn
    )
