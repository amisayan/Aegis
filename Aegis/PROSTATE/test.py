    ...
    def build_model(self):
        if self.model_type == 'Res_Unet':
            self.model = ResUnet(resnet='resnet34', num_classes=self.out_ch, pretrained=True,
                                 mixstyle_layers=[],warmup_epoch=10, num_heads=2, num_class=1, in_planes=512, image_size=384).to(self.device)
        else:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch, data in enumerate(self.test_loader):
                ...
                seg_logit,_ = torch.split(seg_logit, ([seg_logit.shape[0]//2,seg_logit.shape[0]//2]), dim=0)
