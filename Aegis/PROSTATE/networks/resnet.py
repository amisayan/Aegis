...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)
        x_ori1 = x
        sfs_ori.append(x_ori1)
        if 'layer1' in self.mixstyle_layers:
            x = self.random(x)
        sfs.append(x)

        x = self.layer2(x)
        x_ori2 = self.layer2(x_ori1)
        sfs_ori.append(x_ori2)
        if 'layer2' in self.mixstyle_layers:
            x = self.random(x)
        sfs.append(x)

        x = self.layer3(x)
        x_ori3 = self.layer3(x_ori2)
        sfs_ori.append(x_ori3)
        if 'layer3' in self.mixstyle_layers:
            x = self.random(x)
        sfs.append(x)

        x = self.layer4(x)
        x_ori4 = self.layer4(x_ori3)
        sfs_ori.append(x_ori4)
        if 'layer4' in self.mixstyle_layers:
            x = self.random(x)

        return x, sfs, sfs_ori
